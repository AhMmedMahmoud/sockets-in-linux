...
namespace POSIXNetworkSocketLib
{
    ...
    bool Poller::TryAddSender(
        NetworkSocket *communicator, std::function<void()> callback)
    {
        ...
        else if (errno == EEXIST && mReceivers.find(_connectionDescriptor) != mReceivers.end())
        {
        ...
        NetworkSocket *communicator, std::function<void()> callback)
    {
        ...
        else if (errno == EEXIST && mSenders.find(_connectionDescriptor) != mSenders.end())
        {
    ...
    bool Poller::TryRemoveSender(NetworkSocket *communicator)
    {
        // To avoid Linux bug, the event should not be NULL pointer
        int _connectionDescriptor = communicator->Connection();
        struct epoll_event _event;
    ...
    bool Poller::TryRemoveReceiver(NetworkSocket *communicator)
    {
        // To avoid Linux bug, the event should not be NULL pointer
        int _connectionDescriptor = communicator->Connection();
        struct epoll_event _event;
